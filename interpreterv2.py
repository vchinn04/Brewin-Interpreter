...
class Statement(): # In charge of holding and evaluating a single expression
    def __init__(self, val, classDef):
        ...
        if (val[0] in Interpreter.typeTable):
            self.type = Interpreter.typeTable[val[0]]
    ...
    def process_expression(self, vars, classDef, call_dict=None, local_vars=None, get_type=False):
        ...
        if (command == InterpreterBase.PRINT_DEF): # Process the print command
            ...
            for x in curExpr[1:]: # Concatenate all the printed out values into one string
                if (not isinstance(x, Value)):
                    print_val = Value.getVal(Value.getObj(x.process_expression(vars, classDef, local_vars=local_vars), classDef), vars,local_vars, classDef, printV=True)
                else:
...
###################################################################################
            ...
            if (len(curExpr) >= 2):
                if (isinstance(curExpr[1], Class)):
                    ...
                    return curExpr[1].process_expression(vars, classDef, local_vars=local_vars)
                elif (curExpr[1].value == "me"):
                    return classDef
                else:
                    return Value.getVal(curExpr[1], vars, local_vars, classDef)
            else:
                return
###################################################################################
            ...
            if (not isinstance(curExpr[2], Value)):
                secondExpr = curExpr[2].process_expression(vars, classDef, local_vars=local_vars)
            else:
...
###################################################################################
            ...
            if (not isinstance(curExpr[1], Value)):
                firstExpr = Value.getVal(curExpr[1].process_expression(vars, classDef, local_vars=local_vars), vars,local_vars, classDef)
            else:
                firstExpr = Value.getVal(curExpr[1], vars,local_vars, classDef)

            if (not isinstance(curExpr[2], Value)):
                secondExpr = Value.getVal(curExpr[2].process_expression(vars, classDef, local_vars=local_vars), vars,local_vars, classDef)
            else:
                secondExpr = Value.getVal(curExpr[2], vars,local_vars, classDef)
...
###################################################################################
            ...
            if (not isinstance(curExpr[2], Value)):
                methodName = curExpr[2].process_expression(vars, classDef, local_vars=local_vars)
            else:
            ...
            for arg in curExpr[3:]: # Generate a list of the values to be passed in to the called method
                if (isinstance(arg, Class)):
                    ...
                    v = Value.getVal(Value.getObj(arg.process_expression(vars, classDef, local_vars=local_vars), classDef), vars,local_vars, classDef)
                    if (v == None or isinstance(v, Class)):
                        paramList.append(v)
                    else:
                        paramList.append(str(v))
                else:
            ...
            if (isinstance(firstExpr, Class)): # Call the method from the provided class if both exist
                ret_val = firstExpr.method_call(methodName, paramList, isExpr=True, get_type=get_type)
                return ret_val
            elif (firstExpr == InterpreterBase.ME_DEF):
                ret_val = classDef.method_call(methodName, paramList, isExpr=True, get_type=get_type)
                return ret_val
            elif (firstExpr == InterpreterBase.SUPER_DEF):
                ret_val = classDef.method_call(methodName, paramList, callSuper=True, isExpr=True, get_type=get_type)
                return ret_val               
                
            for entr in local_vars:
                if (firstExpr in entr):
                    if (not isinstance(entr[firstExpr]["Value"], Class)):
                        print(entr[firstExpr]["Value"].value)
                        classDef.interpreter.error(ErrorType.FAULT_ERROR)
                    ret_val = entr[firstExpr]["Value"].method_call(methodName, paramList, isExpr=True, get_type=get_type)   
                    return ret_val

            if (firstExpr in vars):
                if (not isinstance(vars[firstExpr]["Value"], Class)):
                    classDef.interpreter.error(ErrorType.FAULT_ERROR)
                ret_val = vars[firstExpr]["Value"].method_call(methodName, paramList, isExpr=True, get_type=get_type)
            elif(firstExpr in classDef.fields):
                if (not isinstance(classDef.fields[firstExpr]["Value"], Class)):
                    classDef.interpreter.error(ErrorType.FAULT_ERROR)
                ret_val = classDef.fields[firstExpr]["Value"].method_call(methodName, paramList, isExpr=True, get_type=get_type)
            else:
                #print("Issue found2!")
...
###################################################################################
            ...
            for entr in local_vars:
                if (firstExpr in entr):
                    ...
                    #print("SET:" , firstExpr, " TO: ", secondExpr)
...
###################################################################################
            ...
            if (firstExpr in local_vars):
                local_vars[firstExpr]["Value"] = Value.getObj(inputStr, classDef)
...
###################################################################################
            ...
            if (firstExpr):
                ...
                if (3 < len(curExpr) and not isinstance(curExpr[3], Value)):
                    ...
                    ret_val = Value.getVal(curExpr[3], vars,local_vars, classDef,loop_call_dict)
...
###################################################################################
            ...
            if (not isinstance(curExpr[1], Value)):
                loop_eval_expr = curExpr[1].process_expression(vars, classDef, local_vars=local_vars)
            else:
...
class StatementBlock(): # Used to store begin blocks
    def __init__(self, val, classDef):
        ...
        self.classDef = classDef
        if (val[0] == "let"): 
            self.local_vars = {} # x[1] : {"Type" : x[0], "Value" : Value.getObj(x[2], classDef)} for x in val[1]
            for x in val[1]:
    ...
    def process_expression(self, vars, classDef, call_dict=None, local_vars=None):
        if (not call_dict):
            call_dict = {}
        ...
        for expr in self.expr:
            if (expr.expression[0] == InterpreterBase.RETURN_DEF): # if its a return expression then set the return flag to true and return
                val = expr.process_expression(vars, classDef, local_vars=local_vars)
                if(call_dict):
                    call_dict["ret"] = True
                local_vars.pop(0)
                ...
                val = expr.process_expression(vars, classDef, call_dict=local_call_dict, local_vars=local_vars) # Process the next expression in block and pass in return flag
...
class Class(): # Class object to create instances of classes
    def __init__(self, class_name, interpreter, sub_class=None):
        if (class_name not in interpreter.classDict.keys()):
        ...
        #print(interpreter.classDict[class_name]["Fields"].items())
    ...
    def compare_class_types(class1, class2, interpreter, class1_obj=None, class2_obj=None ):
       ...
       # print(interpreter.classDict[class_name])
    def method_call(self, method_name, params, callSuper=False, isExpr=False, get_type=False): # Call the method if it exists
        ...
        if (method_name not in self.methods or callSuper):
            ...
            return self.methods[method_name].process_method(self.fields, params, get_type=get_type) 
...
class Method(): # Handles class methods
    ...
    def process_method(self, class_fields, params, get_type=False): # Map passed in values to parameters and throw error if incorrect amount passed in. 
        no_skip = True
        ...
        if (self.params):
            ...
            #for i in self.params:
              #  if (i not in vars):
              #      vars[i]["Value"] = InterpreterBase.NULL_DEF
        else:
            if (len(params) > 0):
                if (self.classDef.BaseClass):
                    no_skip = False
...
class Value():
    def getObj(val, classDef): # Returns the correct object for val depending on its type
        ...
        if (isinstance(val, list)):
            if (val[0] == InterpreterBase.BEGIN_DEF and val[0] != InterpreterBase.LET_DEF):
