...
def generate_test_suite_v1():
    ...
    test_files = [
        file.split(".")[0]
        for file in os.listdir("v1/tests")
        if file.split(".")[1] == "brewin"
    ]
    fail_files = [
        file.split(".")[0]
        for file in os.listdir("v1/fails")
        if file.split(".")[1] == "brewin"
    ]
    return __generate_test_suite(1, test_files, fail_files)
...
def generate_test_suite_v3():
    ...
    test_files = [
        file.split(".")[0]
        for file in os.listdir("v3/tests")
        if file.split(".")[1] == "brewin"
    ]
    fail_files = [
        file.split(".")[0]
        for file in os.listdir("v3/fails")
        if file.split(".")[1] == "brewin"
    ]
